...
import { isReporterName, renderJson, renderJunit, renderMarkdown, writeReports, } from "./reporters/index.js";
...
function parseReporters(values) {
...
function renderReport(report, reporter) {
    if (reporter === "json")
...
function applyBrowserOverrides(config, headed, timeoutMs) {
...
function summaryLine(report) {
    return (`Snippet Fidelity: ${report.summary.passed}/${report.summary.total} passed, ` +
        `${report.summary.failed} failed, ${report.summary.errors} errors.\n`);
}
export async function runCli(args, io = DEFAULT_IO) {
    try {
...
const directEntry = process.argv[1] !== undefined && import.meta.url === pathToFileURL(resolve(process.argv[1])).href;
if (directEntry) {
