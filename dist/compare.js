...
const PUNCTUATION_EQUIVALENTS = new Map([
    ["\u2018", "'"],
    ["\u2019", "'"],
    ["\u201c", '"'],
    ["\u201d", '"'],
...
function terminalLineBreakCount(value) {
    return normalizeLineEndings(value).match(/\n+$/)?.[0].length ?? 0;
}
function stripTerminalLineBreaks(value) {
...
function invisibleCounts(value) {
...
function escapedContext(characters, index) {
    const radius = 12;
...
function findFirstDifference(expected, actual) {
    ...
    let index = 0;
    while (index < limit && expectedCharacters[index] === actualCharacters[index])
        index += 1;
...
function invisibleFinding(expected, actual) {
    ...
    const details = [...changed]
        ...
        .map((codePoint) => `${codePointLabel(codePoint)} ${expectedCounts.get(codePoint) ?? 0}->${actualCounts.get(codePoint) ?? 0}`)
        .join(", ");
    return {
...
export function compareText(expected, actual) {
    ...
    if (stripTerminalLineBreaks(expected) === stripTerminalLineBreaks(actual) &&
    ...
    if (normalizeIndentation(expected) === normalizeIndentation(actual) &&
    ...
    if (normalizeTabs(expected) === normalizeTabs(actual) &&
        normalizedExpected !== normalizedActual) {
        findings.push({
    ...
    if (normalizedExpected.replace(/\s/gu, "") === normalizedActual.replace(/\s/gu, "") &&
        findings.length === 0) {
        findings.push({
